...
export const signInSchema = yup.object().shape({
  ...
  password: yup
    .string()
    .min(8)
    .matches(passwordRules, { message: "Enter a valid password" })
    .required("Required"),
});
...
export const loginSchema = yup.object().shape({
  email: yup
    ...
    .required("Email address is Required for this login method"),
  password: yup
    .string()
    .min(8)
...
export const emailFinderSchema = yup.object().shape({
  ...
  domain: yup
    .string()
    .matches(domainRules, { message: "Enter a valid url" })
    .required("Enter the Company Domain correctly"),
});
...
export const domainNameSchema = yup
  .string()
  .required("Enter The Name You want to search");
